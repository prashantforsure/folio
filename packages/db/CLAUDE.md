...
- **Migrations `0000`–`0039` are applied to the dev Supabase project** and are forward-only.
  ...
  **partial** unique index on `(project_id, idempotency_key)` to nine tables - `characters`,
  ...
  is not here: it counts live `agent_runs` rows, not a window (`startBackgroundRun` /
  `continueBackgroundRun`, under a per-project advisory lock, since task 4.4). RLS block
  ...
  `0036` (the worker, roadmap task 4.1, ADR 0003 D6) is the queue: `job_kind` gains `agent_run` and
  `production_generation`; `jobs` gains `attempts` (a claim adds one), `claimed_at` (the lease
  token) and `heartbeat_at`, two partial indexes (queued by `created_at`, running by
  `heartbeat_at`) and an attempts check; and a hand-written `folio_jobs_notify()` with two triggers
  (`AFTER INSERT`, and `AFTER UPDATE OF status` from anything else) that `pg_notify('folio_jobs',
  kind)` when a row becomes `queued` - drizzle does not model triggers, so `db:check` is blind to
  them. Generated by `db:generate`, renamed, triggers appended. Its SQL and the queue repository
  (`repositories/jobs.ts`) were **exercised against dev inside a rolled-back transaction** on
  2026-09-23 - claim, heartbeat with a cancel and a stale lease, finish twice, requeue, stale
  sweep - and **not applied**. `repositories/jobs.ts` is the one repository that takes the raw
  session database rather than a scope: claiming is across projects by nature. **Trap found
...
- **`0037`** (background runs, roadmap task 4.4) adds `agent_runs.input` (jsonb, null for an
  interactive turn): what a background run was asked to do (`BackgroundRunInputSchema`), which the
  worker rebuilds its context from on every job. Purely additive, generated by `db:generate`,
  renamed, `db:check` clean; **not applied**. Beside it in `repositories/agent.ts`:
  `startBackgroundRun` (chat, brief, run and `agent_run` job in one transaction, refused as `busy` at
  the D14 limit), `continueBackgroundRun`, `cancelBackgroundRun` (the run cancelled at once, its
  queued job cancelled or its running one asked to stop), `beginBackgroundRun` and
