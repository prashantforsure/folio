...
import { DisplayMenu, FilterMenu, STORYBOARD_VIEWS } from './storyboard-toolbar'
 ...
 *
 * The rows, the selection, the filter, the display toggles, the save state
 * and every write live here; `board-view.tsx`, `canvas-view.tsx` and
 * `list-view.tsx` are layouts over `ViewProps` and call back. They share
 * the parts (`shot-parts.tsx`) and share no state, so the three cannot
 * disagree on what a shot is or what it can do.
 *
 ...
 * decision 10). The filter and the display toggles are component state
 * too: a filter is a way of looking, not an address.
 *
 ...
 * shot edit returns the one row; a frame request returns the frame state
 ...
 * redesign - Storyboard is the mode pill's other half, not a sidebar row.
 *
  ...
  return (
    ...
    >
      <div data-storyboard-header data-mounted={mounted ? 'true' : 'false'} className="flex flex-none flex-wrap items-center gap-[10px] px-[20px] pb-[12px] pt-[12px]">
        <ViewPill label="Storyboard views" shape="icon" items={STORYBOARD_VIEWS} current={view} baseHref={baseHref} />
        ...
        <FilterMenu filter={filter} onPick={setFilter} />
