...
import { SLASH_ENTRIES } from '../../../../../../../../lib/outline/slash'
import type { HandleMenuView } from '../../../_script/editor/editor-store'
import { Floating } from '../../../_script/editor/floating/floating-layer'
import { freshBlock, insertBlockAfter } from '../extensions/commands'

/**
 * The `+` handle's menu on the outline: the seven blocks, each inserted
 * *below* the block the handle belongs to, and `Comment`, which opens a
 * new thread under it. The Script route's `handle-menu.tsx` over the
 * outline's rows and commands - the twin, as the slash menus are twins,
 * because the rows and the insert command are each editor's own.
 *
 ...
 */
type Row = { readonly kind: 'block'; readonly type: OutlineNodeType; readonly label: string; readonly glyph: string } | { readonly kind: 'comment' }
...
export const OutlineHandleMenu = ({
  view,
  editor,
  context,
  onComment,
