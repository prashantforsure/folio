 ...
 * before a repository sees it. Membership, not role, as every route's
 * writes are (flagged in `docs/build-decisions.md`, "Open, and blocking").
 *
...
export const addSource = async (projectId: string, rawEdit: unknown): Promise<SourceResult> => {
  ...
  const result = await createResearchSource(gate.scope, edit)
